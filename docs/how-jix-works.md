...
Jix is centered around manifest files (named `__jix__.js`). A jix manifest is a JavaScript file that declaratively defines a desired state for a part of your system. It's composed of one or more interdependent effects.


## The Manifest File

Here is the full content of the `__jix__.js` manifest file. This file would typically be placed in its own directory (e.g., `my-tools/__jix__.js`).

The `jix` object is globally available within the manifest file, so no import is needed.
